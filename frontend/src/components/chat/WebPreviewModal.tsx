...
export const WebPreviewModal: React.FC<WebPreviewModalProps> = ({
  ...
  title: initialTitle = "Web 页面预览",
  fileName: initialFileName = "index.html",
  sessionId,
  isWebContainer: initialIsWebContainer = false,
}) => {
  ...
  const [activeTargetState, setActiveTargetState] = useState<{
    ...
    previewUrl: staticPreviewUrl,
    isWebContainer: initialIsWebContainer,
  });

  const title = activeTargetState.title;
  const fileName = activeTargetState.fileName;
  const currentPreviewUrl = activeTargetState.previewUrl;
  const isWebContainer = activeTargetState.isWebContainer;
  ...
  }, [initialTitle, initialFileName, staticPreviewUrl, initialIsWebContainer]);
  ...
  useEffect(() => {
    if (isOpen && sessionId) {
      api.getSessionFiles(sessionId, token || "")
        ...
        .catch((err) => console.warn("Failed to fetch session files for modal targets", err));
    }
  ...
  useEffect(() => {
    if (isOpen && !isWebContainer && currentPreviewUrl) {
      let isCancelled = false;
      setIsIframeLoading(true);

      async function fetchStaticHtml() {
        try {
          const resp = await fetch(currentPreviewUrl);
          if (resp.ok) {
            let htmlText = await resp.text();
            // 若包含相对路径资源，注入 base 标签
            const urlObj = new URL(currentPreviewUrl, typeof window !== "undefined" ? window.location.origin : "http://localhost:5050");
  ...
  }, [isOpen, isWebContainer, currentPreviewUrl, refreshKey]);
  ...
  const availableTargets = useMemo<PreviewTarget[]>(() => {
    ...
    return targets;
  }, [sessionFiles, sessionId, isWebContainer, title, fileName, currentPreviewUrl]);
  ...
  const activePreviewUrl = isWebContainer ? wcPreviewUrl : currentPreviewUrl;
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-slate-950/85 backdrop-blur-md animate-in fade-in duration-200">
        ...
        >
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="relative" ref={dropdownRef}>
                  <button
                    ...
                    className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-900 hover:bg-slate-800 border border-slate-700/80 text-slate-200 text-xs font-semibold transition-all max-w-[220px] group shadow-sm"
          ...
          <div className="flex-1 w-full bg-slate-950/90 flex flex-row overflow-hidden relative">
            ...
            {/* 页面渲染区域 */}
            <div className="flex-1 h-full flex flex-col items-center justify-center p-2 sm:p-4 overflow-hidden relative">
              <div
