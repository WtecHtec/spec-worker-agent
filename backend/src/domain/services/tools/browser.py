...
def _get_session_id(ctx: dict[str, Any], explicit_session_id: str | None = None) -> str:
    ...
    2. 其次回退使用当前任务上下文的 session_id / task_id
    """
...
class BrowserOpenPageTool(BaseTool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        res = await self.sandbox_client.browser_open(url=url, session_id=session_id, timeout_sec=timeout_sec)
        if not res.get("success"):
            return ToolResult(output=f"打开网页失败: {res.get('error')}", is_error=True)

        raw_output = res.get("output", "页面已打开")
        # 在输出顶部明确标出当前浏览器实例 ID，方便多轮连续感知
...
class BrowserClosePageTool(BaseTool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        session_id = _get_session_id(ctx, browser_instance_id)
        res = await self.sandbox_client.browser_close(session_id=session_id)
        if not res.get("success"):
...
class BrowserGetSnapshotTool(BaseTool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        session_id = _get_session_id(ctx, browser_instance_id)
        res = await self.sandbox_client.browser_get_snapshot(
            session_id=session_id, include_screenshot=include_screenshot
        )
        if not res.get("success"):
...
class BrowserClickTool(BaseTool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        session_id = _get_session_id(ctx, browser_instance_id)
        res = await self.sandbox_client.browser_click(element_id=parsed_id, session_id=session_id)
        if not res.get("success"):
...
class BrowserScreenshotTool(BaseTool):
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        session_id = _get_session_id(ctx, browser_instance_id)
        res = await self.sandbox_client.browser_screenshot(
            full_page=full_page, save_path=save_path, session_id=session_id
        )
        if not res.get("success"):
        ...
        if preview_url and not preview_url.startswith("http"):
            preview_url = f"{self.sandbox_client.base_url.rstrip('/')}{preview_url}"
